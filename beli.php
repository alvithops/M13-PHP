...
if (empty($namacustErr) && empty($emailErr) && empty($notelpErr) && empty($barangPilihErr)) {
    ...
    if ($jumlah == 0) {
        ...
        foreach ($barang_array as $idbarang) {
            ...
            if (!$hasil || mysqli_num_rows($hasil) == 0) {
                ...
                if ($stok < 0) {
                    $simpan = false;
                    $pesan_error = "Stok barang $nama_barang habis!";
                    break;
                }
            }

            // 2. Simpan ke tabel Detail (djual)
            $sql_detail = "INSERT INTO djual (idhjual, idbarang, qty, harga) 
                           VALUES ('$idhjual', '$idbarang', '$qty', '$harga')";
            $hasil_detail = mysqli_query($kon, $sql_detail);

            if (!$hasil_detail) {
                $simpan = false;
                $pesan_error = "Gagal menyimpan detail pembelian";
                break;
            }

            // 3. Update stok barang
            $sql_update = "UPDATE barang SET stok = $stok WHERE id = $idbarang";
            $hasil_update = mysqli_query($kon, $sql_update);

            if (!$hasil_update) {
                $simpan = false;
                $pesan_error = "Gagal mengupdate stok barang";
                break;
            }
        } // End foreach
    }
    ...
    if ($simpan) {
        mysqli_commit($kon);
        // Hapus cookie keranjang karena transaksi sukses
        setcookie('keranjang', '', time() - 3600); 
        header("Location: buktibeli.php?idhjual=$idhjual");
        exit();
    } else {
        mysqli_rollback($kon);
        echo "Pembelian Gagal: " . $pesan_error;
    }

} // End if tidak ada error